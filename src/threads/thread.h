...
// TODO debug code to remove errors that USERPROG is not defined
#ifndef USERPROG
#define USERPROG
#endif
...
#define MAX_OPEN_FILES 128     /* Maximum number of files per thread*/
...
struct thread
  {
    ...
    struct list donations;              /* List of donations of priority. */
    ...
    struct semaphore wait_for_exit;     /* Wait for a thread to exit*/
    struct semaphore wait_for_load;     /* Wait for a thread to load */

#endif

   struct hash *supl_pt;                /* Supplemental page table */
   void* code_segment;                 /* Offset of end of code segment */
...
/* Process structure to keep track of a process */
struct process {
...
bool thread_exists_higher_priority(void);
...
int64_t get_min_wake(void);
bool sleep_list_empty(void);

typedef void thread_action_func (struct thread *t, void *aux);
void thread_foreach (thread_action_func *, void *);

/* Priority scheduling functions */
bool thread_compare_priority(const struct list_elem *, const struct list_elem *, void * UNUSED);
